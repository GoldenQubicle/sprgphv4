class GUI extends PApplet {
  ...
  public void setup() {
    ...
    gearControls = cp5.addAccordion("GC").setPosition(5, 5).setWidth(725).setHeight(800).setCollapseMode(Accordion.SINGLE);
  ...
  void gearGroups(int ls) {
    ...
    for (int gears = 0; gears <= layers.get(layerSelected).gearsGet(); gears++) {
      col+=1;      
      ...
      // setting up new group per gear vector
      cp5.addGroup("gear " + gears).setPosition(10+col*175, 15).setSize(size2d+20, size2d+75).setGroup(cp5.get(Group.class, "row " + row))
  ...
  void gearSlider2D(int gear) {
    cp5.addSlider2D("radius x y " + gear).setGroup( "gear " + gear).setSize(size2d, size2d).setPosition(10, 5).setId(gear)
      .setMinMax(-100, -100, 100, 100).setValue(layers.get(layerSelected).radii.get(gear).x, layers.get(layerSelected).radii.get(gear).y)      
  ...
  void gearPetals(int gear) {
    cp5.addSlider("petals " + gear).setGroup( "gear " + gear).setPosition(13, 175).setSize(size2d, 8).setId(gear)
